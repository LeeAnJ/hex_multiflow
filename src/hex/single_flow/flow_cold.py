...
class HexFlowCold(HexFlow):
    ...
    def calc_out_thsdsdt_with_eps_qact(self, eps: float, q_act_w: float) -> FlowOutletThermalData:
        ...
        q_w = q_act_w * eps                                     # q_act_w > 0 и для хол. потока q_w > 0
        q_jmol = q_w / self.m_mols                              # q_w > 0 и q_jmol > 0
        h_out_jmol = self.inlet.h_jmol + q_jmol                      # h_out_jmol > self.inlet.h для хол. потока
        t_out_k, s_out_jmolk = self.calc_t_k_s_jmolk_with_h_via_isobar(h_jmol=h_out_jmol)
        dt_out_lim = self.outlet_lim.t_k - t_out_k                 # t_out_k < self.outlet_lim.t; cold flows only!!!
        ...
        if dt_out_lim < 0:
            if dt_out_lim < -0.5:
                ...
                sys.exit('cold flow: t_out < t_out_lim in "calc_out_thsdsdt_with_eps_qact". crit.error')
            else:
                t_out_k = self.outlet_lim.t_k

        ds_jmolk = s_out_jmolk - self.inlet.s_jmolk                    # ds_jmolk > 0; cold flows only!!!
    ...
    def calc_out_thsds_limiting_with_eps_qact(self,
        ...
        #
        q_lim_w = q_act_w/eps_hex * eps    # q_lim_w > 0 for cold flow, but q_act_w > 0;
        ...
        h_out_lim_jmol = self.inlet.h_jmol + q_lim_jmol                     # cold flows only!!!
        t_out_lim_k, s_out_lim_jmolk = self.calc_t_k_s_jmolk_with_h_via_isobar(h_jmol=h_out_lim_jmol)
        dt_out_lim = 0
        ds_lim_wk = (s_out_lim_jmolk - self.inlet.s_jmolk)*self.m_mols     # > 0; cold flows only!!!, [W/K]
    ...
    def calc_t_h_s_ds_distribution(self, t_out: float, h_out: float, s_out: float) -> [[], [], []]:  # return t, h, s
        ...
        # т.о. t, h, dq для холодных потоков НЕ НАДО будет реверсировать
        ds = [0.0]
        ...
        dq_sum = dq[-1]
        # print(dq_sum)
        return np.array(t), np.array(h), np.array(s), np.array(ds), np.array(dq), dq_sum
    ...
    def set_eps_for_flows_in_active_pack(self, n_flows_in_pack: int, dlt_eps: float, q_act_w: float) -> []:
        ...
        for i in range(n_epss):
            q_w = q_act_w * eps                     # q_act_w > 0 -> q_w > 0
            q_jmol = q_w/self.m_mols                # q_jmol > 0
            h_out_jmol = self.inlet.h_jmol + q_jmol         # cold flow h_in < h_out
