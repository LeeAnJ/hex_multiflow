...
class HexPackOfHotFlows(HexPackOfFlows):
    def __init__(self, flows: list[HexFlowHot], dlt_eps: float):
    ...
    def calc_dq_w_distribution(self) -> [[], float]:
        # dlt_q_w - с нарастающим эффектом, т.е. dlt_q_w[0]-это все перед.тепло (индексы реверс. для гор.потоков)
    ...
    def get_flows_t_in_lim(self):
        ...
        return max([self.flow[i].inlet.t_k for i in range(self.n_flows)])
